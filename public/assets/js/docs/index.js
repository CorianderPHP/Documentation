...
(() => {
  ...
  var resultContainer = document.querySelector("#docs-search-results");
  var scopeInput = document.querySelector('input[name="scope"]');
  if (searchInput && resultContainer) {
    ...
    const runSearch = () => {
      ...
      fetch(`/api/docs/search?q=${encodeURIComponent(query)}&scope=${encodeURIComponent(scope)}`, { signal: controller.signal }).then((response) => response.ok ? response.json() : null).then((payload) => {
        if (!payload?.results) {
          return;
        }
