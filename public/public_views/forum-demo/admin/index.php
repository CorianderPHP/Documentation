...
<section class="forum-demo-theme relative px-4 py-8 font-poppins sm:px-6 lg:px-8">
    ...
    <div class="mt-4 border-b border-dark-green/15 pb-6 dark:border-peach/20">
        <h1 class="font-concert-one text-4xl text-dark-green dark:text-peach md:text-5xl">Admin moderation</h1>
    ...
    <div class="mt-8 grid gap-4 md:grid-cols-3">
        <div class="border-y border-dark-green/10 py-4 dark:border-peach/15">
            <span class="text-sm text-black/50 dark:text-white/50">Users</span>
            <strong class="mt-1 block text-2xl text-dark-green dark:text-peach"><?= count($users) ?></strong>
        </div>
        <div class="border-y border-dark-green/10 py-4 dark:border-peach/15">
            <span class="text-sm text-black/50 dark:text-white/50">Topics</span>
            <strong class="mt-1 block text-2xl text-dark-green dark:text-peach"><?= count($topics) ?></strong>
        </div>
        <div class="border-y border-dark-green/10 py-4 dark:border-peach/15">
            <span class="text-sm text-black/50 dark:text-white/50">Categories</span>
            <strong class="mt-1 block text-2xl text-dark-green dark:text-peach"><?= count($categories) ?></strong>
        </div>
    </div>

    <section class="mt-10">
        <div class="flex items-center justify-between gap-4 border-b border-dark-green/10 pb-3 dark:border-peach/15">
            <h2 class="font-concert-one text-3xl text-dark-green dark:text-peach">Moderation queue</h2>
            <a href="/forum-demo/admin/users" class="text-sm font-semibold text-dark-green dark:text-peach">Manage users</a>
        </div>
        <div class="divide-y divide-dark-green/10 dark:divide-peach/10">
            <?php foreach ($moderationQueue as $item): ?>
                <div class="grid gap-3 py-5 md:grid-cols-[8rem_1fr_9rem_auto] md:items-center">
                    <span class="text-sm font-semibold capitalize text-dark-green dark:text-peach"><?= htmlspecialchars($item['type'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></span>
                    <div>
                    ...
                    <form method="POST" action="/forum-demo/admin/<?= $item['type'] === 'reply' ? 'replies' : 'topics' ?>" data-demo-form>
                        ...
                        <button name="action" value="<?= htmlspecialchars($item['action'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>" class="rounded-md border border-dark-green/25 px-3 py-2 text-sm font-semibold text-dark-green dark:border-peach/30 dark:text-peach"><?= htmlspecialchars(ucfirst($item['action']), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></button>
                    </form>
                </div>
            <?php endforeach; ?>
        </div>
