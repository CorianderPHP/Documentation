...
<section class="forum-demo-theme relative px-4 py-8 font-poppins sm:px-6 lg:px-8">
    <div class="flex flex-col gap-4 border-b border-dark-green/15 pb-6 dark:border-peach/20 md:flex-row md:items-end md:justify-between">
        <div>
    ...
    <div class="mt-8 grid gap-8 lg:grid-cols-[1fr_22rem]">
        <section>
            <div class="hidden border-b border-dark-green/10 pb-2 text-xs font-semibold uppercase tracking-1 text-black/45 dark:border-peach/15 dark:text-white/45 md:grid md:grid-cols-[1fr_8rem_7rem]">
                <span>Topic</span>
                <span>Updated</span>
                <span>Replies</span>
            </div>
                    ...
                    <a href="/forum-demo/topics/<?= (int) $topic['id'] ?>" class="grid gap-3 px-2 py-5 transition hover:bg-black/5 md:grid-cols-[1fr_8rem_7rem] md:items-center">
                        <div>
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span class="rounded-full border border-dark-green/15 px-2 py-0.5 text-xs font-semibold dark:border-peach/20"><?= htmlspecialchars($topic['status'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></span>
                            </div>
                        ...
                        <span class="text-sm font-semibold text-dark-green dark:text-peach"><?= (int) $topic['replies'] ?> replies</span>
                    </a>
                <?php endforeach; ?>
            </div>
        </section>
            ...
            <?php else: ?>
                <form method="POST" action="/forum-demo/topics" data-demo-form class="mt-4 space-y-4">
                    ...
                    <label class="block">
                        ...
                        <select name="category_id" class="mt-1 w-full rounded-md border border-dark-green/20 bg-white px-3 py-2 text-black dark:border-peach/30 dark:bg-black dark:text-white">
                    ...
                    <label class="block">
                        ...
                        <input name="title" class="mt-1 w-full rounded-md border border-dark-green/20 bg-white px-3 py-2 text-black dark:border-peach/30 dark:bg-black dark:text-white" placeholder="How do migrations work?">
                    </label>
                    <label class="block">
                        <span class="text-sm font-semibold">Original post</span>
                        <textarea name="body" rows="5" class="mt-1 w-full rounded-md border border-dark-green/20 bg-white px-3 py-2 text-black dark:border-peach/30 dark:bg-black dark:text-white"></textarea>
                    </label>
                    <button class="w-full rounded-md bg-dark-green px-4 py-2 font-semibold text-true-white dark:bg-peach dark:text-black">Create topic</button>
                </form>
            <?php endif; ?>
        </aside>
    </div>
