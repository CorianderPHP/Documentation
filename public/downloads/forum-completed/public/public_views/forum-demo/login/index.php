...
<section class="forum-demo-theme relative mx-auto max-w-4xl px-4 py-10 font-poppins sm:px-6">
    ...
    <h1 class="mt-4 font-concert-one text-4xl text-dark-green dark:text-mint md:text-5xl">Demo login</h1>
    ...
    <div class="mt-8 grid gap-5 md:grid-cols-2">
        <?php foreach ($demoAccounts as $account): ?>
            <form method="POST" action="/forum-demo/login" class="rounded-lg border border-dark-green/15 bg-true-white p-5 shadow-sm dark:border-mint/20 dark:bg-true-black">
                <?= \CorianderCore\Core\Security\Csrf::input() ?>
                <input type="hidden" name="quick_role" value="<?= $account['role'] ?>">
                <h2 class="font-concert-one text-3xl text-dark-green dark:text-mint"><?= $account['label'] ?></h2>
                ...
                <button class="mt-5 w-full rounded-md bg-dark-green px-4 py-2 font-semibold text-true-white dark:bg-mint dark:text-black">Use <?= $account['label'] ?></button>
            </form>
        <?php endforeach; ?>
    </div>

    <form method="POST" action="/forum-demo/login" class="mt-8 rounded-lg border border-dark-green/15 bg-true-white p-5 shadow-sm dark:border-mint/20 dark:bg-true-black">
        <?= \CorianderCore\Core\Security\Csrf::input() ?>
        <h2 class="font-concert-one text-3xl text-dark-green dark:text-mint">Manual credentials</h2>
        <div class="mt-5 grid gap-4 md:grid-cols-2">
            <label class="block">
                <span class="text-sm font-semibold">Email</span>
                <input name="email" type="email" class="mt-1 w-full rounded-md border border-dark-green/20 bg-white px-3 py-2 text-black dark:border-mint/30 dark:bg-black dark:text-white">
            </label>
            <label class="block">
